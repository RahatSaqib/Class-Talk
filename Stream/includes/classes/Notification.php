...
class Notification {
	...
	public function getNotifications($data, $limit,$msg_by_code) {
		...
		$query = mysqli_query($this->con, "SELECT * FROM notifications WHERE class_code='$msg_by_code' ORDER BY id DESC");
...
if($row['user_to']==$userLoggedIn){
	...
	public function insertNotification($post_id, $user_to, $type,$msg_by_code) {
		...
		switch($type) {
			...
			case 'profile_comment':
				...
				break;
			case 'assignment':
				$message = $userLoggedInName . " assigned a task";
				break;
				case 'Assignment':
				$message = $userLoggedInName . " commented on a task";
				break;
		}
		if($type=='Assignment'){
			$link = "view_assignment.php?id=" . $post_id;
			// $link='';
		}
		else if($type=='assignment'){
			// $link='assignment';
			$link = "assign_assignment.php?as_id=" . $post_id;
		}
		else{
		...
		$insert_query = mysqli_query($this->con, "INSERT INTO notifications VALUES('', '$user_to', '$userLoggedIn', '$message', '$link', '$date_time', 'no', 'no','$msg_by_code')");
	}
